...
if (isset($_POST['submit'])) {
    ...
    if (!$title) {
        ...
        if ($thumbnail['name']) {
            ...
            // rename img
            $time = time(); // make each img name upload unique using current time stamp
            ...
            if(in_array($extension, $allowed_files)) {
              // make sure img is not to big 2mb
              if($thumbnail['size'] < 2_000_000) {
                //upload thumbnail
                move_uploaded_file($thumbnail_tmp_name, $thumbnail_destination_path);
              } else {
                $_SESSION['edit-post'] = "Thumbnail size is too big. Should be less than 2mb";
              }
    ...
    if ($_SESSION['edit-post']) {
        ...
        // set thumbnail name if a new one was uploaded, else keep lod thumbnail name
